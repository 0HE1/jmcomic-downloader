...
async function refreshApiDomains() {
  const result = await commands.refreshApiDomains()
  if (result.status === 'error') {
    message.error(`刷新动态API域名失败：${result.error}`)
    return
  }
  message.success(`刷新动态API域名成功，当前使用：${result.data}`)
}
</script>
    ...
    <n-dialog class="w-140!" :showIcon="false" title="配置" @close="showing = false">
      <div class="flex flex-col">
        ...
        <div class="mt-1 flex justify-end">
          <n-button size="small" @click="refreshApiDomains">刷新动态API域名</n-button>
        </div>
