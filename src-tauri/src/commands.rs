...
#[tauri::command(async)]
#[specta::specta]
pub async fn refresh_api_domains(app: AppHandle) -> CommandResult<String> {
    let jm_client = app.get_jm_client();

    match jm_client.update_api_domains_force().await {
        Ok(()) => {
            let config = app.get_config();
            let config = config.read();
            let current_domain = config.get_effective_api_domain();
            Ok(current_domain)
        }
        Err(err) => Err(CommandError::from("刷新动态API域名失败", err)),
    }
}
